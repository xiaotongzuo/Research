...
mm.set_nu(True)                   # for Ring Mover   # turn off!!!
...
# Job Distributor
jd=PyJobDistributor("1207output_ring",1,scorefxn)
...
while not jd.job_complete:
    ...
    for i in range(1):                   # min mover,50, 5%, 150
        for ii in range(5):               # shear+ring,19, 95%
            smallmover.apply(pose)
            pmm.apply(pose)
            shearmover.apply(pose)
            pmm.apply(pose)
